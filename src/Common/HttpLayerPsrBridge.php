...
final class HttpLayerPsrBridge extends HttpLayer
{
    ...
    public function get(string $uri, array $body = []): array
    {
        ...
        $resp = $this->psrLayer->get($uri, $query);
        return $this->compatResponse($resp);
    }

    /** @return array{status_code:int,headers:array<string,string[]>,body:mixed,response:ResponseInterface} */
    public function post(string $uri, array $body = []): array
    {
        $resp = $this->psrLayer->post($uri, $body);
        return $this->compatResponse($resp);
    }
    ...
    {
        $resp = $this->psrLayer->put($uri, $body);
        return $this->compatResponse($resp);
    }
    ...
    public function request(string $method, string $uri, string $body = ''): array
    {
        ...
        return match ($m) {
            'GET'    => $this->get($uri, []),
            'POST'   => $this->post($uri, $body !== '' ? (array) json_decode($body, true) : []),
            'PUT'    => $this->put($uri, $body !== '' ? (array) json_decode($body, true) : []),
